...
import { LocationIcon, PropertyTypeIcon, OccupantStatusIcon, PropertyStatusIcon, BuildYearCalendarIcon, WhitePlusIcon } from '@/layout/svgIconPaths';
    ...
    <div className='bg-[#fafafe] px-3 sm:px-4 md:px-6 lg:px-10 w-full'>
      ...
      <section className='flex flex-col justify-center'>
        ...
        <div className='flex flex-wrap justify-center items-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-[12.71px] p-2 md:p-[6.35px] mt-2 rounded-[7.62px] shadow-lg sm:shadow-xl md:shadow-2xl'>
          <Dropdown options={['Lagos', 'Abia', 'Benue', 'Benin']} placeholder='Location' icon={<LocationIcon />}/>
          <Dropdown options={['Bungalow', '3-Storey', 'Duplex', 'Hall']} placeholder='Property Type' icon={<PropertyTypeIcon />} />
          <Dropdown options={['Occupied', 'Not Occupied']} placeholder='Occupant Status' icon={<OccupantStatusIcon />} />
          <Dropdown options={['Rental', 'Outright Sell', 'Landed Property']} placeholder='Property Status' icon={<PropertyStatusIcon />} />
          <Dropdown options={['2024', '2025', '2001', '1992']} placeholder='Build Year' icon={<BuildYearCalendarIcon />} />
        </div>
      </section>

      {/* Properties List Section */}
      <section className='mt-6 sm:mt-8 md:mt-12 lg:mt-16 w-full'>
        <div className='text-[#4D4D4D] font-[600] mt-4 sm:mt-6 md:mt-10 text-[16px] sm:text-[18px] md:text-[22px] leading-[145%]' style={{fontFamily: 'Plus Jakarta Sans'}}>
          Properties List
        </div>
        <div className={`flex ${useColumnLayout ? 'flex-col' : 'flex-col lg:flex-row'} gap-4 sm:gap-6 w-full flex-wrap`} style={{fontFamily: 'Plus Jakarta Sans'}}>
      ...
      <section className='flex flex-col gap-4 sm:gap-6 mt-6 sm:mt-8 md:mt-10'>
        {/* Header and Button in Separate Row on Mobile */}
        <div className='flex flex-col sm:flex-row justify-between items-start w-full'>
          <div className='text-[#4D4D4D] font-[600] text-[16px] sm:text-[18px] md:text-[22px] leading-[145%]' style={{fontFamily: 'Plus Jakarta Sans'}}>
