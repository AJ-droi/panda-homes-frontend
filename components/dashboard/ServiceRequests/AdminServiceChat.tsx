...
//     });
//   };
let tenantServiceDetail = messages.find((item) => item.sender === 'tenant') as any

  return (
    <div className="max-w-7xl mx-auto bg-white min-h-screen w-full">
      {/* Header */}
      <div className="py-4 px-2 border-y border-[#CACDD8] flex items-center gap-x-2">
        <BackButton />
        <h1 className="text-md md:text-xl font-medium text-gray-900 mb-2">
         {toSentenceCase(tenantServiceDetail?.serviceRequest?.tenant?.user?.first_name)}  {toSentenceCase(tenantServiceDetail?.serviceRequest?.tenant?.user?.last_name)} [{`${tenantServiceDetail?.content}`}]
        </h1>
              ...
              <div
                className={`p-4 rounded-lg ${
                  message.sender === "admin"
                    ...
                    : "bg-[#000] text-[#fff] rounded-ss-2xl rounded-es-2xl rounded-ee-2xl rounded-se-none"
                }`}
              >
                <p className="text-sm md:text-md leading-relaxed">{message.content}</p>
              </div>
